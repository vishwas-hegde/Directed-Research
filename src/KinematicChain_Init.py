...
def Benchmark(Map_Num,Range,Goal_Bias,Proj_Links):
    '''
    # Info:
    # si: Space Infomation
    ...
    if benchmarking == 'ON':
        ...
        Command = 'ompl_benchmark_statistics.py Map_'+str(Map_Num)+'_'+str(round(planner.getRange(),2))+'_'+str(planner.getGoalBias())+'.log -d PSO_database_'+str(Map_Num)+'.db'
        os.system(Command)
        pass
        # db = "./benchmark.db"
...
if __name__ == "__main__":
    f = open("/home/dhrumil/Git/Directed-Research/src/PSO.json")
    json_data = json.load(f)
    for key in json_data:
        Range = float(json_data[key]['Range'])
        Goal_Bias = float(json_data[key]['Bias'])
        Proj_Links = int(json_data[key]['Proj'])
        Benchmark(int(key),Range,Goal_Bias,Proj_Links)
